...
watch(
  () => search.value?.commandPaletteRef?.query,
  debounce((query: string) => {
    ...
    useTrackEvent("Search", {
      props: {
        query: `${query} - ${search.value?.commandPaletteRef.results.length} results`,
      },
    });
  }, 500),
);
  ...
  <div>
    ...
    >
      <div class="flex items-center gap-1">
        ...
        />
        <span
          ><span class="font-semibold">Nuxt Nation</span> conference is coming.
          Join us on November 12-13.</span
        >
        <UButton
          label="Sign up"
          color="white"
